...
class TestCreateBot:
    ...
    @patch("discord_against_humanity.bot.create_repo_factory")
    @patch("discord_against_humanity.bot.valkey.Valkey")
    def test_returns_bot_instance(self, mock_valkey, mock_factory):
        bot = create_bot()
        assert isinstance(bot, Bot)

    @patch("discord_against_humanity.bot.create_repo_factory")
    @patch("discord_against_humanity.bot.valkey.Valkey")
    def test_command_prefix(self, mock_valkey, mock_factory):
        bot = create_bot()
        assert bot.command_prefix == "!"

    @patch("discord_against_humanity.bot.create_repo_factory")
    @patch("discord_against_humanity.bot.valkey.Valkey")
    def test_intents_message_content(self, mock_valkey, mock_factory):
        bot = create_bot()
        assert bot.intents.message_content is True

    @patch("discord_against_humanity.bot.create_repo_factory")
    @patch("discord_against_humanity.bot.valkey.Valkey")
    def test_intents_members(self, mock_valkey, mock_factory):
        bot = create_bot()
        assert bot.intents.members is True

    @patch("discord_against_humanity.bot.create_repo_factory")
    @patch("discord_against_humanity.bot.valkey.Valkey")
    def test_valkey_attribute(self, mock_valkey, mock_factory):
        bot = create_bot()
    ...
    @patch("discord_against_humanity.bot.create_repo_factory")
    @patch("discord_against_humanity.bot.valkey.Valkey")
    def test_repo_factory_attribute(self, mock_valkey, mock_factory):
        bot = create_bot()
        assert hasattr(bot, "repo_factory")
        mock_factory.assert_called_once()

    @patch("discord_against_humanity.bot.create_repo_factory")
    @patch("discord_against_humanity.bot.valkey.Valkey")
    @patch.dict("os.environ", {"VALKEY_HOST": "db.example.com", "VALKEY_PORT": "6380"})
    def test_valkey_uses_env_vars(self, mock_valkey, mock_factory):
        create_bot()
    ...
    def test_valkey_defaults(self, mock_valkey, mock_factory):
        create_bot()
        mock_valkey.assert_called_once_with(
