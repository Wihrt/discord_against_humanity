...
class TestBlackCard:
    ...
    def test_collection(self, mock_valkey_client):
        card = BlackCard(mock_valkey_client)
    ...
    async def test_create_without_document_id(self, mock_valkey_client):
        card = await BlackCard.create(mock_valkey_client)
    ...
    async def test_create_with_document_id(self, mock_valkey_client):
        ...
        card = BlackCard(mock_valkey_client, repository=mock_repo)
...
class TestWhiteCard:
    ...
    def test_collection(self, mock_valkey_client):
        card = WhiteCard(mock_valkey_client)
    ...
    async def test_create_without_document_id(self, mock_valkey_client):
        card = await WhiteCard.create(mock_valkey_client)
    ...
    async def test_create_with_document_id(self, mock_valkey_client):
        ...
        card = WhiteCard(mock_valkey_client, repository=mock_repo)
        await card.get(doc_id)
        assert card._document["_id"] == doc_id
